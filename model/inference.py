...
def main():
    ...
    model.load_weights(torch.load("/fs/nexus-projects/audio-visual_dereverberation/data/pretrained-models/dereverb/vida.pth", map_location='cpu'))
    # else:
    #     print('Warning: no pretrained model weights are found!')
    model.to(device=torch.device('cuda'))
    model.eval()
    with torch.no_grad():
        rgb = np.array(Image.open("/fs/nexus-projects/audio-visual_dereverberation/input_1.png"))
        depth = np.array(Image.open("/fs/nexus-projects/audio-visual_dereverberation/input_2.png"))
        audio, _ = librosa.load("/fs/nexus-projects/audio-visual_dereverberation/input_1.wav", sr=16000)
        audio_length = 40960
        ...
        batch = {'rgb': transform(image=rgb)['image'].unsqueeze(0),
                'depth': transform(image=depth)['image'].unsqueeze(0),
        ...
        output = model.dereverb(batch, 0, phase='test')
        pred, tgt = output['pred'], output['tgt']
        print(pred.shape, tgt.shape)
        sf.write("/fs/nexus-projects/audio-visual_dereverberation/output_1.wav", pred.cpu().numpy()[0], 16000)
